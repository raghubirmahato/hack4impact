...
const Admin: React.FC = () => {
  ...
  const pending = data.doctors.filter(doctor => doctor.verificationStatus === 'pending' || !doctor.isVerified);
  ...
  const setDoctorStatus = (id: string, isVerified: boolean) => {
    write('goodhealth_doctors', data.doctors.map(doctor => doctor.id === id ? { ...doctor, isVerified, isAvailable: isVerified, verificationStatus: isVerified ? 'approved' : 'rejected', updatedAt: new Date().toISOString() } : doctor));
    refresh();
  };
  const setPatientStatus = (id: string, isActive: boolean) => {
    write('goodhealth_users', data.patients.map(patient => patient.id === id ? { ...patient, isActive, verificationStatus: isActive ? 'approved' : 'rejected', updatedAt: new Date().toISOString() } : patient));
    refresh();
  };
  ...
  return <main className="min-h-screen pt-24 pb-12 px-4 sm:px-6"><div className="max-w-7xl mx-auto">
    ...
    {tab === 'patients' && <section className="panel overflow-hidden"><TableHead labels={['Patient', 'Email', 'Joined', 'Verification']}/>{data.patients.length ? data.patients.map(patient => <div key={patient.id} className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 border-t border-white/10 text-sm items-center"><span className="text-white">{patient.name}</span><span className="truncate">{patient.email}</span><span>{new Date(patient.createdAt).toLocaleDateString()}</span><div className="flex gap-2 items-center"><Status status={patient.verificationStatus || (patient.isActive ? 'approved' : 'pending')}/><button onClick={() => setPatientStatus(patient.id, !patient.isActive)} className="justify-self-start text-xs px-3 py-1.5 rounded-full bg-white/10 hover:bg-white/20">{patient.isActive ? 'Suspend' : 'Approve'}</button></div></div>) : <Empty label="No patient records found."/>}</section>}
  </div></main>;
};
